...
public class MainPresenter implements Presenter, android.view.View.OnClickListener, SwipeRefreshLayout.OnRefreshListener,
    ...
    private UNote.NoteType mCurrentNoteTypePage = UNote.NoteType.getDefault();
    ...
    public void onCreate(Bundle savedInstanceState) {
        if (savedInstanceState != null){
            ...
            mCurrentNoteTypePage = UNote.NoteType.mapValueToStatus(value);
        }
        view.initToolbar();
        initDrawer();
        initMenuGravity();
    ...
    public void onDrawerItemSelect(int position){
        mCurrentNoteTypePage = UNote.NoteType.mapValueToStatus(position);
    ...
    private boolean onMenuItemActionCollapse(){
        if (mCurrentNoteTypePage != UNote.NoteType.TRASH){
    ...
    public void switchNoteTypePage(UNote.NoteType type){
    ...
    private void startNoteActivity(int type, UNote value){
    ...
    public void onRecyclerViewItemClick(int position, UNote value){
        if (mCurrentNoteTypePage == UNote.NoteType.TRASH){
            return;
        }
    ...
    public void showPopMenu(android.view.View v, int position, UNote value){
        if (mCurrentNoteTypePage == UNote.NoteType.TRASH){
    ...
    private void moveToTrash(UNote note){
        if (note == null)
            return;
        note.setType(UNote.NoteType.TRASH);
        note.setStatus(UNote.Status.NEED_REMOVE);
        mFinalDb.update(note);
        view.removeNote(note);
        pushNote(note);
    }

    private void recoverNote(UNote note){
        if (note == null)
            return;
        note.setType(UNote.NoteType.NORMAL);
        note.setStatus(UNote.Status.NEED_PUSH);
        mFinalDb.update(note);
        view.removeNote(note);
        pushNote(note);
    }

    public void onDeleteForeverDialogClick(UNote note, int which){
    ...
    public boolean onPopupMenuClick(int id, UNote note){
        switch (id){
            case R.id.edit:
    ...
    private void pushNote(UNote note){
    ...
    public void onEventMainThread(NotifyEvent event){
        switch (event.getType()){
            ...
            case NotifyEvent.CREATE_NOTE:
                if (event.getData() instanceof UNote){
                    UNote note = (UNote)event.getData();
                    view.addNote(note);
